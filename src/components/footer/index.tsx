  ...
  <footer className={tw(`bg-white border-t border-gray-400 pt-14 pb-16`)}>
    <div className={tw(`max-w-7xl mx-auto text-gray-400 px-8 lg:px-0 flex flex-wrap`)}>
      <div className={tw(`mb-14 flex items-center w-full`)}>
        <img className={tw(`h-22 w-22 mr-4`)} src="logo.png" alt="logo" width={128} height={128} />
      </div>
      <div className={tw(`w-full lg:w-5/5`)}>
        <ul className={tw(`text-lg font-light flex flex-wrap w-full`)}>
          ...
          <li className={tw(`w-1/2 md:w-1/3 lg:w-1/5`)}>
            <div>
              <h4 className={tw(`text-gray-900 text-base font-bold mb-1`)}>SellerSea For</h4>
              <ul>
                {sellerFor.map((link) => (
                  <li className={tw(`text-gray-800 text-sm font-medium leading-6`)} key={link}>
                    <a href="/">{link}</a>
